...
fn main() {
    ...
    loop {
        ...
        // Check if we've seen this signature hash in the last TIME_WINDOW seconds
        ...
        if current_time - last_seen_time >= TIME_WINDOW {
            // This is a new log with a new signature, print it out
            println!("{}", line);
            seen_signatures.insert(signature_hash_hex, current_time);
        } else {
            // This is a duplicate log with a recently seen signature, increment the "seen" counter
            seen_signatures.insert(signature_hash_hex, current_time);
